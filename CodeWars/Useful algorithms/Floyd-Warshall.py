...
def paths_eval(p, n):
    ...
    index = len(p) - 1
    while index > 0:
        removals = [x for x in p if p.index(x) < index and x[0] == p[index][0] and x[-1] == p[index][-1]]
        for x in removals: p.remove(x)
        index -= len(removals) + 1
    for i in range(0, len(p)):
        ...
        for j in range(0, len(p)):
            ...
            while index < len(points) - 1:
                if p[j][0] == points[index] and p[j][-1] == points[index + 1]:
                    points.insert(index + 1, p[j][2])
                    break
                else: index += 1
        p[i] = ','.join(points)
    for i in range(1, n + 1):
        ...
        endpointsfound = [x[-1] for x in p if x[0] == str(i)]
    ...
    while index <= n:
        ...
        px = [x for x in p if x[0] == str(index)]
        p = [x for x in p if x not in px]
        px.sort(key=lambda k: int(k[-1]))
        p += px
        index += 1
    return p
